...
def sliding_window_opt(args):
    ...
    for i in range(P_org.shape[0]):
        ...
        # 修復 V 的定義，移除對 cam_params_est 的依賴
        V = np.array([np.array([0])])  # 模板0（即下一個圖像）可以被圖像0（當前圖像）看到
...
def sliding_window_sz2_opt(args):
    ...
    for i in range(P_org.shape[0] - 1):
        ...
        if V.size > 0:
            ...
            print("Warning: V array is empty, skipping deletion")
            V = np.array([np.array([0])])  # 創建一個默認值
